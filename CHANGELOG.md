...
### Fixed
- **浮动面板「放大/缩小」改为调整窗口尺寸**：右下角/边缘可拖拽拉伸，按钮按比例调窗口大小；内容保持自然尺寸重排（原为内容 CSS 缩放，放大后文字与图溢出窗口）。
- **「⏹ 终止」停不下来修复**：会话驱动生成以 `queue` 排队，而平台 cancel 只停当前回合、队列 FIFO 续跑——终止后进入 20s「武装窗口」自动续掐每个续跑回合直到排空，会话忙碌期间新的图/讲解请求拒绝入队并提示（原可无限堆积，停止后仍逐条继续执行）。
- **🎨 动态出图场景按工作区隔离**：场景内存表此前是进程级 `Map<figureId>`，A 工作区画的
  图会出现在 B 工作区的面板与场景列表里（内存读取优先于按 root 的磁盘路径）。现按 root
  嵌套键控（`Map<root, Map<figureId>>`），`dynamic-N` 内存 id 也不再跨工作区碰撞；暂存
  槽从单槽改为按 figId 注册（场景归属在暂存时确定），两个工作区并发出图不再互相踩掉
  暂存（此前后画的会顶掉先暂存的槽，导致先画的图丢失捕获）。磁盘保存路径本就按 root
  隔离，不受影响。
- **讲解暂存与会话图缓存同样按会话/工作区隔离**：讲解暂存（notePending）此前是进程级
  单槽——任一会话暂存讲解后，另一会话再暂存即顶掉前者，先暂存会话的回答丢失讲解信封
  捕获与用量归账；现按会话 id 键控（带 30 分钟 TTL）。一次工作区的重新扫描此前会清掉
  **所有**工作区的内存图缓存（`graphCaches.clear()`），现只失效本 root。其余状态经全量
  审计确认本就按 root/figId 键控（图暂存、生成活动、追问中断、索引/文档 in-flight），
  各 tab 图的磁盘缓存读写始终按 root 隔离。
