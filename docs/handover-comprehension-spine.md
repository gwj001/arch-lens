...
- 章讲解落盘：`staged.chapter` 存在时写 `.arch-lens-explain-<章>-<语言>.json`
  ...
  `docchapter.chapterExplainDeps` 从该章图缓存经 `figureDeps` 派生（er/catalog 无图
  → undefined=legacy 全失效）；`requires`=导出的 `CHAPTER_REQUIRES`（图就地重生 →
  既有 `invalidateRequiring` 级联墓碑讲解）。
...
- [ ] `chapterExplainDeps`：seq/交互→from/to 与生产者/消费者包；依赖→核心 ids；flow→两视角并集；ER/目录→undefined（legacy 全失效）。
...
**V2① 细粒度包依赖**（备忘 §四.3/§八）：需先把每章事实块收窄到相关包，才敢把章节
`deps` 从"全部包"窄化（否则欠失效、服务过期正文）。信封格式已就位，勿二次改格式。

**可选项**：讲解信封的章覆盖补全——客户端组件/事件/概念讲解暂不落章（粒度不对齐）；
er 章无独立 tab（与 deps 同源）。

## 九、一句话总结

主干 = 认知序（阶段 0）→ 旧稿当底稿改（阶段 1）→ 上游结论下传 + 消费即依赖的失效账
（阶段 2）→ 学习过的章节文档白得（阶段 3）。四阶段已提交未推送；V2① 是下一站；
改任何缓存/提示词前，先回到备忘与本文 §四/§五 对齐不变式。
