...
| `（本批）` | **V2①：细粒度包依赖**——事实块与信封 `deps` 同源收窄（时序/交互/依赖三章），无关包变动只重盖章、相关包变动才失效；讲解信封 deps 同源（覆盖黄金路径，无欠失效边角） |
...
- 收窄三章（子集源自嵌入图数据）：`seq`→消息 from/to 包；`interaction`→事件包 ∪
  黄金路径包（`interactionScope`）；`deps`→核心 ids。ER/目录/概念/流程**保持全量**
  （全局事实是它们的本质）——失效方向永远安全：没喂的包不会失效章节，喂了的必失效。
- 信封 `deps` = `chapterPackageDeps(kind, figureCache, graph)`，与事实块**同源**派生；
  讲解信封 deps（`chapterExplainDeps`）同源。无欠失效边角。
...
**细粒度包依赖（V2①）**
- [ ] 时序/交互/依赖章事实块（包清单 + 调用边表）只含其消费的包；交互章覆盖黄金路径端点。
- [ ] 收窄章的 `deps` 与事实块同源（时序=[消息端点]、交互=[事件∪黄金路径]、依赖=[核心 ids]）；全局章（ER/目录/概念/流程）=全量。
...
**主干四阶段与 V2① 全部落地**。可选项：
- **讲解信封覆盖补全**：客户端组件/事件/概念讲解暂不落章（粒度不对齐）；er 章无独立 tab（与 deps 同源）。
- **流程章事实收窄**：flow 保持全量（mermaid 节点→包映射脆）；若流程图节点规范化为包 id，可收窄。
- **并行化**：仅同层无依赖节点（如流程两视角）可选，与主干串行哲学相抵。
