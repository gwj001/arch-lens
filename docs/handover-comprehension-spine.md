...
| `（本批）` | **deps-union 防御**：正文引用的真实包必进信封 deps（`citedPackages`），欠失效边角的确定性兜底 |
| `（本批）` | **flow n×n 交叉视角修正**：doc 流程块只锚定实体×事件一格（`!methods && angle==='event'`），实体×管道走档案投影、方法两格走各自归纳；旧 flow 缓存已清理 |
...
- 信封 `deps` = `chapterPackageDeps(kind, figureCache, graph, index?)`，与事实块**同源**
  派生（er 需要 index，无 index 回退全量）；**deps-union 防御**：落信封时并入正文
  引用的真实包（`citedPackages`：反引号 token ∩ ground truth，与门禁同一提取）——
  `DELETE-gone-names` 是提示词级契约，这是确定性兜底（正常生成时幂等）；讲解信封
  deps（`chapterExplainDeps`）同源。无欠失效边角。
...
- [ ] **deps-union 防御**：正文/讲解引用的真实包（反引号 ∩ ground truth）必进信封 deps——模型违反 DELETE-gone-names 保留收窄面外引用时，该包变动仍失效该章。
