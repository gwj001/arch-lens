...
async function listSrc(fs, base) {
	try {
		...
		return (await fs.listDir(src)).filter((entry) => entry.type === "file").map((entry) => entry.name).slice(0, MAX_FILES_PER_NODE);
	} catch {
		return [];
	}
}
...
async function resolveOpt(fs, base, name) {
	try {
...
async function isFile(fs, base, name) {
...
async function isDir(fs, base, name) {
	const target = await resolveOpt(fs, base, name);
	if (target === null) return false;
	try {
		const info = await fs.stat(target);
...
async function listEntries(fs, dir) {
	try {
...
async function childDirs(fs, dir) {
	return (await listEntries(fs, dir)).filter((entry) => entry.type === "directory" && !SKIP_DIRS$1.has(entry.name));
}
/** Resolve a directory to its target, or null. */
async function dirTarget(fs, base, name) {
	const target = await resolveOpt(fs, base, name);
	if (target === null) return null;
	try {
		const info = await fs.stat(target);
		if (info === void 0 || info.type !== "directory") return null;
		return target;
	} catch {
		return null;
	}
...
*/
async function collectManifestDirs(fs, root, manifests) {
	const out = [];
	const walk = async (dir, depth) => {
		...
		for (const entry of entries) {
			if (entry.type !== "directory" || SKIP_DIRS$1.has(entry.name)) continue;
...
async function hasSourceFile(fs, dir, extension, depth) {
	const entries = await listEntries(fs, dir);
...
async function collectFiles(fs, dir, extension, maxDepth) {
	const out = [];
	...
	const walk = async (current, depth) => {
		...
		for (const entry of entries) {
			...
			} else if (entry.type === "directory" && !SKIP_DIRS$1.has(entry.name)) await walk(entry.target, depth + 1);
		}
	};
	await walk(dir, 0);
	return out.sort();
...
async function detectScanLanguage(fs, root) {
	if (await isFile(fs, root, "package.json")) return "typescript";
	for (const manifest of PYTHON_MANIFESTS) if (await isFile(fs, root, manifest)) return "python";
	for (const manifest of JAVA_MANIFESTS) if (await isFile(fs, root, manifest)) return "java";
...
async function readmeBlurb(fs, dir) {
...
async function componentDetail(fs, node, dependents) {
	...
	if (entryName !== void 0 && node.files.includes(entryName)) {
		const readName = lang === void 0 || lang === "typescript" ? `src/${entryName}` : entryName;
		const head = await readHead(fs, node.path, readName, 12e3);
		snippet = head.split("\n").slice(0, 90).map((line) => line.length > 140 ? `${line.slice(0, 137)}…` : line).join("\n");
		const pattern = lang === "typescript" || lang === void 0 ? /(ctx\.(on|provide|effect|emit|waterfall|serial|parallel|inject)\(|\.register\(|harness\.(handle|registerTool)\(|@Remote\()/ : /^\s*@[A-Za-z]/;
		...
		if (entryRef !== void 0) entryRef.role = "entry";
	}
...
* @returns the graph.
*/
async function scanTypeScript(fs, root) {
	const nodes = [];
	const edges = [];
	...
	if (await isDir(fs, root, "packages")) {
		const groupEntries = (await listEntries(fs, await dirTarget(fs, root, "packages"))).filter((entry) => entry.type === "directory");
	...
	if (nodes.length === 0) {
		...
		if (rootMeta !== null) await addPackage(root, "", rootMeta);
...
async function distName(fs, dir) {
	const pyproject = await readHead(fs, dir, "pyproject.toml", MAX_HEAD_BYTES);
	return (pyproject !== "" ? pyprojectName(pyproject) : null) ?? basenameOf(dir);
}
...
async function pythonSplitCandidates(fs, distDir) {
	const srcTarget = await dirTarget(fs, distDir.displayPath, "src");
	const topLevel = [];
	if (srcTarget !== null) {
		for (const entry of await childDirs(fs, srcTarget)) if (await hasSourceFile(fs, entry.target, ".py", 1)) topLevel.push(entry);
	} else for (const entry of await childDirs(fs, distDir)) if (await hasSourceFile(fs, entry.target, ".py", 1)) topLevel.push(entry);
	if (topLevel.length === 1) {
		const importChildren = (await childDirs(fs, topLevel[0].target)).filter((entry) => entry.name !== "__pycache__");
		const coded = [];
		for (const child of importChildren) if (await hasSourceFile(fs, child.target, ".py", 1)) coded.push(child);
...
async function scanPython(fs, root) {
	...
	const unitDirs = [];
	if (codeDists.length >= 2 || codeDists.length === 1 && codeDists[0] !== root) for (const dir of codeDists) {
		const target = await dirTarget(fs, dir, ".");
		if (target !== null) unitDirs.push({
			name: basenameOf(dir),
			type: "directory",
			target
		});
		...
		const distTarget = await dirTarget(fs, root, ".");
		if (distTarget !== null) unitDirs.push({
			name: basenameOf(root),
			type: "directory",
			target: distTarget
		});
...
function pomDependencyIds(text) {
...
async function javaFiles(fs, dir) {
	...
	return (prefixed.length > 0 ? prefixed : names).slice(0, MAX_FILES_PER_NODE);
}
/** Read a file's FULL text (bounded); '' when missing/oversized. */
async function readWhole(fs, base, name) {
	try {
		const target = await fs.resolve(childPath(base, name));
		const info = await fs.stat(target);
		if (info === void 0 || info.type !== "file" || info.size !== void 0 && info.size > MAX_HEAD_BYTES) return "";
...
async function springProfile(fs, dir, files) {
	const profile = {
		stereotypes: [],
		feignClients: [],
		listeners: [],
	...
	const priority = [...files].sort((a, b) => {
		return (/(Application|Controller|Service|Client|Listener|Consumer|Producer|Config)\.java$/.test(a) ? 0 : 1) - (/(Application|Controller|Service|Client|Listener|Consumer|Producer|Config)\.java$/.test(b) ? 0 : 1) || a.localeCompare(b);
	});
	let scanned = 0;
	const stereotypes = /* @__PURE__ */ new Set();
	for (const name of priority) {
		...
		if (profile.main === void 0 && text.includes("@SpringBootApplication")) profile.main = base;
		...
		if (feign !== null) {
			...
			if (target !== void 0 && target !== "") profile.feignClients.push(target);
		}
		for (const regex of [/@KafkaListener\s*\(([\s\S]*?)\)/g, /@RabbitListener\s*\(([\s\S]*?)\)/g]) {
			...
			while ((listener = regex.exec(text)) !== null) {
				...
				if (strings !== null) for (const s of strings.slice(0, 4)) profile.listeners.push(s.slice(1, -1));
			}
		}
...
async function findAppRootDir(fs, srcMainJava) {
	const all = await collectFiles(fs, srcMainJava, ".java", 12);
	if (all.length === 0) return null;
	const heads = {};
	const readHeadOf = async (rel) => {
		if (heads[rel] === void 0) heads[rel] = await readHead(fs, srcMainJava.displayPath, rel, 65536);
		return heads[rel];
	};
	...
	for (const rel of [...named, ...all.filter((rel) => !named.includes(rel))]) {
		if ((await readHeadOf(rel)).includes("@SpringBootApplication")) {
	...
	for (let depth = 0; depth < 10; depth += 1) {
		...
		const dirs = entries.filter((entry) => entry.type === "directory" && !SKIP_DIRS$1.has(entry.name));
...
async function javaNodesForModule(fs, moduleDir) {
	const nodes = [];
	const srcMainJava = await dirTarget(fs, moduleDir, "src/main/java");
	if (srcMainJava === null) return {
		nodes,
		edges: []
	};
	...
	const layerDirs = [];
	for (const candidate of layerCandidates) if (await hasSourceFile(fs, candidate.target, ".java", 3)) layerDirs.push(candidate);
	const directJava = (await listEntries(fs, appRoot)).filter((entry) => entry.type === "file" && entry.name.endsWith(".java"));
	...
	if (layerDirs.length >= 2 || layerDirs.length === 1 && directJava.length === 0) for (const layer of layerDirs) {
...
async function scanJava(fs, root) {
	const pomDirs = await collectManifestDirs(fs, root, JAVA_MANIFESTS);
	let moduleDirs = [];
	if (pomDirs.length > 0) {
		for (const dir of pomDirs) if (await isDir(fs, dir, "src/main/java")) moduleDirs.push(dir);
	} else {
		const walk = async (dir, depth) => {
			if (depth > 2) return;
	...
	const nodes = [];
	const edges = [];
	const artifactAliases = /* @__PURE__ */ new Map();
	if (moduleDirs.length >= 2) {
		for (const dir of moduleDirs) {
			if (await dirTarget(fs, dir, "src/main/java") === null) continue;
		...
		for (const dir of moduleDirs) {
			...
			for (const dep of pomDependencyIds(pomText)) {
				...
				if (targetId !== null && targetId !== fromId) {
					...
					if (fromNode !== void 0 && !fromNode.deps.includes(targetId)) {
	...
	if (nodes.length > 0) for (const node of nodes) {
		...
		for (const target of targets) {
			const match = nodes.find((candidate) => candidate.id !== node.id && (normId(candidate.id) === normId(target) || normId(basenameOf(candidate.path)) === normId(target)));
			if (match !== void 0 && !edges.some((edge) => edge.from === node.id && edge.to === match.id)) edges.push({
				from: node.id,
				to: match.id
			});
		}
...
function nodeIdForArtifact(artifact, nodes, aliases) {
	const want = normId(artifact);
	for (const node of nodes) {
		if (normId(node.id) === want) return node.id;
		for (const alias of aliases.get(node.id) ?? []) if (normId(alias) === want) return node.id;
	}
	return null;
}
/** Scan a workspace with no recognized manifest: one root node, never error. */
async function scanUnknown(fs, root) {
	const id = basenameOf(root);
	const { blurb, blurbZh } = await readmeBlurb(fs, root);
	const nodes = [{
		id,
...
async function scanWorkspace(fs, root) {
	try {
		switch (await detectScanLanguage(fs, root)) {
			case "typescript": return await scanTypeScript(fs, root);
			case "python": return await scanPython(fs, root);
			case "java": return await scanJava(fs, root);
			default: return await scanUnknown(fs, root);
		}
	} catch (error) {
...
function scanGroupLabel(graph, group) {
	if (graph.lang === void 0 || group !== "") return groupLabel(group);
...
function cacheName$6(base, language, methods = false) {
...
function seqCacheName(language, methods = false) {
	return cacheName$6(SEQ_CACHE$1, language, methods);
}
...
function eventsCacheName(language, methods = false) {
	return cacheName$6(EVENTS_CACHE, language, methods);
}
/**
...
async function readStructuredCache(fs, root, language, kind, methods = false) {
	try {
		const parsed = await readVersionedCache(fs, await fs.resolve(cacheName$6(kind === "seq" ? SEQ_CACHE$1 : EVENTS_CACHE, language, methods), { cwd: root }), await readFactVersion(fs, root));
...
function buildSequenceFromCalls(index, language) {
	...
	for (const pkg of index.packages) {
		for (const entity of pkg.entities) fileToPkg.set(norm$1(entity.file), pkg.id);
		for (const imp of pkg.imports) fileToPkg.set(norm$1(imp.from), pkg.id);
	}
	const fileImports = /* @__PURE__ */ new Map();
	for (const pkg of index.packages) for (const imp of pkg.imports) {
		const list = fileImports.get(norm$1(imp.from)) ?? [];
		list.push({
			to: imp.to,
			names: imp.names
		});
		fileImports.set(norm$1(imp.from), list);
	}
	const resolveModule = (spec, fromFile) => {
		if (spec.startsWith("./") || spec.startsWith("../")) {
			...
			for (const candidate of candidates) {
				const pkg = fileToPkg.get(norm$1(candidate));
	...
	for (const edge of calls) {
		if (isTestFile$1(norm$1(edge.fromFile))) continue;
		const callerPkg = fileToPkg.get(norm$1(edge.fromFile));
		if (callerPkg === void 0) continue;
		const imports = fileImports.get(norm$1(edge.fromFile)) ?? [];
		...
		const calleePkg = resolveModule(module, norm$1(edge.fromFile));
		if (calleePkg === void 0 || calleePkg === callerPkg) continue;
		const key = `${callerPkg}\u0000${calleePkg}`;
		const existing = edges.get(key);
		if (existing !== void 0) {
			existing.syms.add(edge.to);
			if (existing.file === void 0) existing.file = norm$1(edge.fromFile);
		} else edges.set(key, {
			to: calleePkg,
			syms: /* @__PURE__ */ new Set([edge.to]),
			file: norm$1(edge.fromFile)
		});
	}
...
function packagePath(pkg, root) {
	...
	if (firstEntity !== void 0) return norm$1(firstEntity.file);
	return norm$1(pkg.path).replace(norm$1(root), "").replace(/^\/+/, "");
}
/**
...
function figureDeps(kind, data, index) {
	...
	const workspaceWide = index === void 0 || index.language === "typescript" ? all : void 0;
	switch (kind) {
		case "concepts": return workspaceWide;
		case "flow-event":
		case "flow-pipeline": return data?.source === "doc" ? [] : workspaceWide;
		case "seq": {
			const messages = Array.isArray(data) ? data : data?.messages;
			if (!Array.isArray(messages)) return workspaceWide;
		...
		case "duties": return typeof data === "object" && data !== null ? Object.keys(data) : workspaceWide;
	}
}
/**
...
async function summarizeDuties(ctx, fs, root, graph, language, sandboxPolicy) {
	...
	for (const batch of missingBatches.slice(0, MAX_BATCHES_PER_CALL)) {
		const lines = graph.nodes.filter((node) => batch.includes(node.id)).map((node) => `- ${node.id}: ${node.blurb !== "" ? node.blurb : "（无自带描述，请依据其源码/文件判断职责）"}`).join("\n");
...
* @param root - absolute workspace root.
*/
async function compareWorkspaceChanges(fs, root) {
...
async function commitWorkspaceManifest(fs, root, files, sandboxPolicy) {
...
function packageOfRel(rel, known, nodes, root) {
	...
	if (nodes !== void 0 && root !== void 0 && nodes.length > 0) {
...
*/
function computeChangedPackages(fileChanges, oldIds, newIds, context) {
...
//#endregion
//#region packages/arch-lens-backend/src/generation-activity.ts
...
*/
function TrackGeneration(kind) {
	return function(method, context) {
		if (context.static || context.private || typeof context.name !== "string") throw new TypeError("arch-lens: @TrackGeneration requires a public instance method with a string name");
		return function(...args) {
			...
			try {
				return Promise.resolve(method.apply(this, args)).finally(() => host.generationActivity.exit(root, kind));
			} catch (error) {
...
let ArchLensService = (() => {
	...
	return class ArchLensService extends _classSuper {
		static {
			...
			__esDecorate(this, null, _remoteGenerationActive_decorators, {
				kind: "method",
				name: "remoteGenerationActive",
				static: false,
				private: false,
				access: {
					has: (obj) => "remoteGenerationActive" in obj,
					get: (obj) => obj.remoteGenerationActive
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			...
			__esDecorate(this, null, _remoteFigurePending_decorators, {
				kind: "method",
				name: "remoteFigurePending",
				static: false,
				private: false,
				access: {
					has: (obj) => "remoteFigurePending" in obj,
					get: (obj) => obj.remoteFigurePending
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			...
			__esDecorate(this, null, _remoteDynamicFigureFailed_decorators, {
				kind: "method",
				name: "remoteDynamicFigureFailed",
				static: false,
				private: false,
				access: {
					has: (obj) => "remoteDynamicFigureFailed" in obj,
					get: (obj) => obj.remoteDynamicFigureFailed
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
		...
		* cache read per root; a read of another root can run alongside. */
		graphInFlight = null;
		/** Staged explain questions PER SESSION (notePending → the next matching
		* answer), keyed by the staging session id (empty string = defensive bucket
		* for stagings without one). The pre-fix SINGLE slot was stomped by the
		* next staging from ANY session — the first session's answer then found no
		* staging, and its explain capture (chapter envelope + usage attribution)
		* was lost. 30-minute TTL per entry, same defensive bound as the figure
		* stagings. */
		pendingNotes = /* @__PURE__ */ new Map();
		...
		* the first. Entries are removed on figId match or 30-min TTL. */
		pendingFigures = /* @__PURE__ */ new Map();
		...
		* while an LLM call happens to be streaming. */
		...
		/** Per-figId staged CUSTOM figure requests (🎨 动态出图 / 🔧 修复重画):
		* matched by figId in the agent's answer and captured into the STAGING
		* root's scene store. figId-keyed — NOT a single slot — so concurrent draws
		* in different workspaces (or rapid re-draws) never stomp each other's
		* staging (a stomped slot lost the first answer: its figId no longer
		* matched). `root` is resolved at stage time: the scene belongs to the
		* workspace whose facts built the prompt, even if the user switches away
		* mid-turn. */
		pendingCustomFigures = /* @__PURE__ */ new Map();
		/** All custom figures known this session, PER WORKSPACE ROOT then keyed by
		* scene id: generated by the panel OR restored from disk. `saved` reflects
		* whether the CURRENT content is persisted (a follow-up re-render flips it
		* back to false). Nested per root — the flat pre-fix map leaked scenes
		* across workspaces (a figure drawn in workspace A showed in B's panel and
		* scene list, and `dynamic-N` memory ids collided across roots). */
		customFigures = /* @__PURE__ */ new Map();
		/** The scene map for one root, created on first WRITE (read paths use
		* `get(root)` and tolerate undefined so they never grow the outer map). */
		ensureFigureMap(root) {
			let map = this.customFigures.get(root);
			if (map === void 0) {
				map = /* @__PURE__ */ new Map();
				this.customFigures.set(root, map);
			}
			return map;
		}
		...
		*/
		async remoteRefresh() {
			...
			if (!fileChanges.changed) {
				const graph = await this.graph();
				if (graph === null) {} else if ("error" in graph) return graph;
				else {
			...
			this.graphCaches.delete(root);
			if (this.graphInFlight?.root === root) this.graphInFlight = null;
			try {
				const target = await this.ctx.fs.resolve(GRAPH_CACHE_FILE, { cwd: root });
				await this.ctx.fs.writeText(target, JSON.stringify({
					root,
		...
		async remoteFigurePending() {
			...
			for (const pending of this.pendingFigures.values()) {
				...
				out.push({
					...
					...pending.dynamic !== void 0 ? { dynamic: pending.dynamic } : {}
				});
			}
		...
		*/
		async remoteDynamicFigureFailed(request) {
			const root = this.resolveRoot();
			if (typeof root !== "string") return root;
			const kind = request.kind === "seq-edge" ? "seq-edge" : request.kind === "overview" ? "overview" : "flow-subgraph";
			const language = request.language ?? "中文";
		...
		*/
		async remoteCustomFigurePrompt(request) {
			...
			try {
				...
				const existing = figureId !== void 0 ? this.customFigures.get(root)?.get(figureId) ?? await this.readDrawFromDisk(root, figureId) : null;
				...
				const usageStart = this.sessionUsageSnapshot(this.targetSessionId);
				this.pendingCustomFigures.set(figId, {
					figId,
					figureId,
					text,
					language,
				...
				setTimeout(() => {
					this.pendingCustomFigures.delete(figId);
				}, 1800 * 1e3);
				return {
					figId,
					figureId,
		...
		*/
		async remoteCustomFigure(request) {
			...
			if (request.figureId !== void 0 && request.figureId !== "") {
				const mem = this.customFigures.get(root)?.get(request.figureId);
			...
			let newestMem = null;
			for (const entry of this.customFigures.get(root)?.values() ?? []) if (newestMem === null || entry.at > newestMem.at) newestMem = {
		...
		* never get reused. */
		async allocateFigureId(root) {
			...
			for (const key of this.customFigures.get(root)?.keys() ?? []) {
		...
		*/
		async remoteCustomFigureList() {
			...
			try {
				...
				for (const entry of this.customFigures.get(root)?.values() ?? []) byId.set(entry.figureId, {
					figureId: entry.figureId,
					title: entry.title,
					text: entry.text,
					saved: entry.saved
		...
		*/
		async remoteFigureRepairPrompt(request) {
			...
			try {
				const scene = this.customFigures.get(root)?.get(request.figureId) ?? await this.readDrawFromDisk(root, request.figureId);
		...
		*/
		async remoteSaveCustomFigure(request) {
			...
			const result = this.customFigures.get(root)?.get(request.figureId);
		...
		*/
		async remoteCustomFigureDelete(request) {
			...
			try {
				...
				this.customFigures.get(root)?.delete(request.figureId);
				return { ok: true };
			} catch (error) {
		...
		*/
		async remoteNotePending(request) {
			if (request.text === "") {
				this.pendingNotes.delete(request.sessionId ?? "");
				return { ok: true };
			}
			const usageStart = this.sessionUsageSnapshot(request.sessionId ?? null);
			const stagedKey = request.sessionId ?? "";
			this.pendingNotes.set(stagedKey, {
			...
			setTimeout(() => {
				this.pendingNotes.delete(stagedKey);
			}, 1800 * 1e3);
			return { ok: true };
		}
		/**
		...
		*/
		...
		async [(_remoteGraph_decorators = [Remote("graph")], _remoteRefresh_decorators = [Remote("refresh")], _remoteRefreshIndex_decorators = [Remote("refreshIndex")], _remoteGenerateAll_decorators = [TrackGeneration("all"), Remote("generateAll")], _remoteSetSession_decorators = [Remote("setSession")], _remoteComponent_decorators = [Remote("component")], _remoteNotes_decorators = [Remote("notes")], _remoteMermaidDeps_decorators = [Remote("mermaidDeps")], _remoteMermaidEr_decorators = [Remote("mermaidEr")], _remoteMermaidIndexed_decorators = [Remote("mermaidIndexed")], _remoteCallGraph_decorators = [Remote("callGraph")], _remoteMermaidCore_decorators = [Remote("mermaidCore")], _remoteOverviewFigure_decorators = [Remote("overviewFigure")], _remoteConceptTree_decorators = [Remote("conceptTree")], _remoteGenerateDocs_decorators = [TrackGeneration("docs"), Remote("generateDocs")], _remoteSequence_decorators = [Remote("sequence")], _remoteRegenerateFigure_decorators = [TrackGeneration("figure"), Remote("regenerateFigure")], _remoteLastAnswer_decorators = [Remote("lastAnswer")], _remoteGenerationStatus_decorators = [Remote("generationStatus")], _remoteGenerationStatusNext_decorators = [Remote("generationStatusNext")], _remoteGenerationActive_decorators = [Remote("generationActive")], _remoteFigurePrompt_decorators = [Remote("figurePrompt")], _remoteDynamicFigurePrompt_decorators = [Remote("dynamicFigurePrompt")], _remoteFigurePending_decorators = [Remote("figurePending")], _remoteDynamicFigure_decorators = [TrackGeneration("dynamic"), Remote("dynamicFigure")], _remoteDynamicFigureFailed_decorators = [Remote("dynamicFigureFailed")], _remoteCustomFigurePrompt_decorators = [Remote("customFigurePrompt")], _remoteCustomFigure_decorators = [TrackGeneration("custom"), Remote("customFigure")], _remoteCustomFigureList_decorators = [Remote("customFigureList")], _remoteFigureRepairPrompt_decorators = [Remote("figureRepairPrompt")], _remoteSaveCustomFigure_decorators = [Remote("saveCustomFigure")], _remoteCustomFigureDelete_decorators = [Remote("customFigureDelete")], _remoteFigureFollowUp_decorators = [TrackGeneration("followup"), Remote("figureFollowUp")], _remoteCancelFollowUp_decorators = [Remote("cancelFollowUp")], _remoteCancelGeneration_decorators = [Remote("cancelGeneration")], _remoteEvents_decorators = [Remote("events")], _remoteFlow_decorators = [Remote("flow")], _remoteAnalyze_decorators = [Remote("analyze")], _remoteSummarizeDuties_decorators = [TrackGeneration("duties"), Remote("summarizeDuties")], _remoteProgress_decorators = [TrackGeneration("progress"), Remote("progress")], _remoteProgressStats_decorators = [Remote("progressStats")], _remoteLlmStats_decorators = [Remote("llmStats")], _remoteNotePending_decorators = [Remote("notePending")], _remotePromptConfig_decorators = [Remote("promptConfig")], _remotePromptConfigSave_decorators = [Remote("promptConfigSave")], Service.init)]() {
			this.ctx.on("session/event", (session, event) => {
				...
				if (this.pendingCustomFigures.size > 0) for (const stagedCustom of this.pendingCustomFigures.values()) {
					const parsed = extractFigureJson(answer, stagedCustom.figId);
					if (parsed === null) continue;
					this.pendingCustomFigures.delete(stagedCustom.figId);
					...
					if (value !== void 0) {
						this.ensureFigureMap(stagedCustom.root).set(stagedCustom.figureId, {
						...
						console.log(`[arch-lens] custom figure ${stagedCustom.figureId} captured (${stagedCustom.root}, memory only, not persisted)`);
					}
					break;
				}
				const stagedKey = this.pendingNotes.has(session.id) ? session.id : "";
				const staged = this.pendingNotes.get(stagedKey);
