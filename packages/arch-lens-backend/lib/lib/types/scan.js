    ...
    try {
        ...
        for (const group of groupEntries) {
            ...
            for (const pkg of pkgEntries) {
                ...
                const blurb = firstParagraph(readme);
                const files = await listSrc(fs, base);
                nodes.push({ id: short, short, group: group.name, blurb, files, deps, path: base });
            }
        }
    }
    catch (error) {
 ...
 */
export async function componentDetail(fs, graph, node) {
