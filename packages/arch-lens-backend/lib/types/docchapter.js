...
import { checkDocProse, citedPackages, formatViolations } from "./doc-hallucination.js";
...
export async function chapterExplainDeps(fs, root, kind, language) {
    const pkgIds = (subset) => subset !== undefined && subset.size > 0 ? [...subset] : undefined;
    switch (kind) {
        case 'concepts':
            return undefined;
        case 'seq':
        ...
        case 'interaction': {
            const events = await readStructuredCache(fs, root, language, 'interaction');
...
function sharedFacts(index, graph, duties, withEdges, core = null, pkgSubset) {
    ...
    for (const node of graph.nodes.slice(0, MAX_PACKAGE_LINES)) {
        if (pkgSubset !== undefined && !pkgSubset.has(node.id))
            continue;
    ...
    if (withEdges) {
        ...
        outer: for (const [from, targets] of importEdges(index)) {
            if (pkgSubset !== undefined && !pkgSubset.has(from))
                continue;
            for (const to of targets) {
                if (pkgSubset !== undefined && !pkgSubset.has(to))
                    continue;
                rows.push(`| ${from} | ${to} |`);
 ...
 */
export function chapterPackageDeps(kind, cache, graph, index) {
    const all = graph.nodes.map(node => node.id);
    const scoped = (subset) => subset !== undefined && subset.size > 0 ? [...subset] : null;
    switch (kind) {
        case 'seq':
 ...
 */
export async function generateDocChapter(ctx, fs, root, kind, language, facts, truth, factsVersion, chapterDeps, figureBlocks = '', sandboxPolicy, priorMarkdown = '', requires = []) {
    const title = chapterTitle(kind, language);
