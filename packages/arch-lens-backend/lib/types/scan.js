...
async function collectManifestDirs(fs, root, manifests) {
    ...
    const walk = async (dir, depth) => {
        ...
        if (manifests.some(manifest => names.has(manifest)))
            out.push(dir.displayPath);
        for (const entry of entries) {
            if (entry.type !== 'directory' || SKIP_DIRS.has(entry.name))
                continue;
...
async function hasSourceFile(fs, dir, extension, depth) {
    const entries = await listEntries(fs, dir);
    if (entries.some(entry => entry.type === 'file' && entry.name.endsWith(extension)))
        return true;
    if (depth <= 0)
        return false;
    for (const entry of entries) {
        if (entry.type !== 'directory' || SKIP_DIRS.has(entry.name))
            continue;
        if (await hasSourceFile(fs, entry.target, extension, depth - 1))
            return true;
    }
    return false;
}
...
async function collectFiles(fs, dir, extension, maxDepth) {
    ...
    const walk = async (current, depth) => {
        if (out.length >= MAX_SOURCE_COLLECT || depth > maxDepth)
            return;
        const entries = await listEntries(fs, current);
        for (const entry of entries) {
            if (out.length >= MAX_SOURCE_COLLECT)
                return;
            if (entry.type === 'file') {
                if (entry.name.endsWith(extension))
                    out.push(relUnder(base, entry.target.displayPath));
            }
...
async function detectScanLanguage(fs, root) {
    if (await isFile(fs, root, 'package.json'))
        return 'typescript';
    for (const manifest of PYTHON_MANIFESTS) {
        if (await isFile(fs, root, manifest))
            return 'python';
    }
    ...
    if (await isFile(fs, root, 'build.gradle') || await isFile(fs, root, 'build.gradle.kts'))
        return 'java';
    return 'unknown';
}
/* ------------------------------------------------------------------ */
...
async function pythonSplitCandidates(fs, distDir) {
    ...
    if (srcTarget !== null) {
        for (const entry of await childDirs(fs, srcTarget)) {
            if (await hasSourceFile(fs, entry.target, '.py', 1))
                topLevel.push(entry);
        }
    }
    else {
    ...
    if (topLevel.length === 1) {
        ...
        for (const child of importChildren) {
            if (await hasSourceFile(fs, child.target, '.py', 1))
                coded.push(child);
        }
        if (coded.length >= 2)
            return coded;
...
async function scanPython(fs, root) {
    ...
    for (const dir of dists) {
        ...
        if (target !== null && await hasSourceFile(fs, target, '.py', 2))
            codeDists.push(dir);
    }
    const unitDirs = [];
    if (codeDists.length >= 2 || (codeDists.length === 1 && codeDists[0] !== root)) {
        for (const dir of codeDists) {
            const target = await dirTarget(fs, dir, '.');
            if (target !== null)
                unitDirs.push({ name: basenameOf(dir), type: 'directory', target });
        }
    }
        ...
        if (distTarget !== null) {
            ...
            if (split.length > 0)
                ...
                unitDirs.push({ name: basenameOf(root), type: 'directory', target: distTarget });
        }
    }
    else {
...
function pomDependencyIds(text) {
    ...
    while ((match = block.exec(cleaned)) !== null) {
        ...
        let inner;
        while ((inner = artifact.exec(match[0])) !== null)
            ids.add(inner[1].trim());
    }
    return [...ids];
}
...
async function springProfile(fs, dir, files) {
    const profile = { stereotypes: [], feignClients: [], listeners: [], endpoints: [] };
    ...
    for (const name of priority) {
        if (scanned >= MAX_SPRING_SCAN_FILES)
            break;
        scanned += 1;
        ...
        if (profile.main === undefined && text.includes('@SpringBootApplication'))
            profile.main = base;
        for (const line of text.split('\n')) {
            const match = JAVA_STEREOTYPE.exec(line);
            if (match !== null)
                stereotypes.add(match[1]);
        }
        ...
        if (feign !== null) {
            ...
            if (target !== undefined && target !== '')
                profile.feignClients.push(target);
        }
            ...
            while ((listener = regex.exec(text)) !== null) {
                ...
                if (strings !== null) {
                    for (const s of strings.slice(0, 4))
                        profile.listeners.push(s.slice(1, -1));
                }
            }
        }
        ...
        let endpoint;
        while ((endpoint = mappings.exec(text)) !== null)
            profile.endpoints.push(endpoint[1]);
        if (profile.endpoints.length >= 6)
            break;
        if (profile.feignClients.length >= 4 && profile.listeners.length >= 4 && profile.endpoints.length >= 4)
            break;
    }
...
async function findAppRootDir(fs, srcMainJava) {
    ...
    const readHeadOf = async (rel) => {
        if (heads[rel] === undefined)
            heads[rel] = await readHead(fs, srcMainJava.displayPath, rel, 65536);
        return heads[rel];
    };
    ...
    for (const rel of [...named, ...all.filter(rel => !named.includes(rel))]) {
        ...
        if (named.length === 0 && Object.keys(heads).length >= 120)
            break;
    }
    ...
    for (let depth = 0; depth < 10; depth += 1) {
        ...
        if (directFiles.length > 0 || dirs.length !== 1)
            return current;
...
async function javaNodesForModule(fs, moduleDir) {
    ...
    for (const candidate of layerCandidates) {
        if (await hasSourceFile(fs, candidate.target, '.java', 3))
            layerDirs.push(candidate);
    }
...
async function scanJava(fs, root) {
    ...
    if (pomDirs.length > 0) {
        for (const dir of pomDirs) {
            if (await isDir(fs, dir, 'src/main/java'))
                moduleDirs.push(dir);
        }
        ...
        const walk = async (dir, depth) => {
            if (depth > 2)
                return;
            if (await isDir(fs, dir.displayPath, 'src/main/java'))
                moduleDirs.push(dir.displayPath);
            for (const entry of await childDirs(fs, dir))
                await walk(entry.target, depth + 1);
        };
    ...
    const artifactAliases = new Map(); // nodeId → own artifactIds
    if (moduleDirs.length >= 2) {
        for (const dir of moduleDirs) {
            ...
            if (pomText !== '') {
                ...
                if (own !== null && own !== '')
                    artifactAliases.set(id, [...(artifactAliases.get(id) ?? []), own]);
            }
...
function nodeIdForArtifact(artifact, nodes, aliases) {
    ...
    for (const node of nodes) {
        if (normId(node.id) === want)
            return node.id;
        for (const alias of aliases.get(node.id) ?? []) {
            if (normId(alias) === want)
                return node.id;
        }
    }
    return null;
