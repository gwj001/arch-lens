...
export function buildSequenceFromCalls(index, language) {
    ...
    for (const edge of calls) {
        ...
        for (const imp of imports) {
            if (imp.names.includes(edge.to) || imp.names.includes(root)) {
                module = imp.to;
                break;
            }
        }
        ...
        if (existing !== undefined)
            ...
            edges.set(key, { to: calleePkg, syms: new Set([edge.to]) });
    }
    if (edges.size === 0)
        return null;
...
export async function readSeqCache(fs, root, language) {
    try {
...
export async function writeSeqCache(fs, root, language, result, sandboxPolicy) {
    const target = await fs.resolve(cacheName(SEQ_CACHE, language), { cwd: root });
