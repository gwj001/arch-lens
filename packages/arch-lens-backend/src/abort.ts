...
const slots = new WeakMap<AbortSignal, { startedAt: number; seq: number; lastNotify: number; status: GenerationStatus }>()
const waiters = new Map<AbortSignal, Array<() => void>>()
 ...
 */
export function generationSignal(root: string): AbortSignal {
 ...
 */
export function abortGeneration(root: string): boolean {
  const existing = controllers.get(root)
...
function slotFor(signal: AbortSignal): { startedAt: number; seq: number; lastNotify: number; status: GenerationStatus } {
...
function notify(signal: AbortSignal): void {
 ...
 */
export function beginGenerationStage(signal: AbortSignal | undefined, stage: string): void {
 ...
 */
export function reportGeneration(signal: AbortSignal | undefined, outputChars: number, preview: string): void {
  if (signal === undefined) return
  const slot = slotFor(signal)
  slot.seq += 1
  slot.status = {
...
export function endGenerationStage(signal: AbortSignal | undefined): void {
  if (signal === undefined) return
  const slot = slotFor(signal)
  slot.seq += 1
...
export function tailPreview(accumulated: string, delta: string): string {
 ...
 */
export function currentGenerationStatus(root: string): GenerationStatus | null {
...
export async function waitForGenerationStatus(
...
): Promise<{ status: GenerationStatus; seq: number } | null> {
  const controller = controllers.get(root)
  if (controller === undefined) return null
  ...
  const slot = slotFor(signal)
  if (slot.seq !== since) return { status: slot.status, seq: slot.seq }
