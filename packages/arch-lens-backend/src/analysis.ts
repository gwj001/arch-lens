...
import { FLOW_ANGLE_LABEL, FLOW_STYLE_RULES, flowAngleRule, sanitizeMermaid } from './flow-angle.ts'
...
const inflight = new Map<string, Promise<ArchLensAnalysisProfile>>()
...
const currentProfiles = new Map<string, ArchLensAnalysisProfile>()
...
const mutations = new Map<string, Promise<ArchLensAnalysisProfile>>()

/** Drop every in-flight profile and mutation (rescan invalidates the
 * analysis layer too). */
export function clearAnalysisProfileCache(): void {
  inflight.clear()
 ...
 */
export async function ensureAnalysisProfile(
  ctx: Context,
  fs: FileSystem,
  root: string,
...
async function resolveProfile(
  ...
  const figures = structure.coreIds !== undefined && structure.coreIds.length >= MIN_CORE_IDS
    ...
    : null
  const profile: ArchLensAnalysisProfile = {
...
export function profileFromText(text: string): ArchLensAnalysisProfile | null {
  try {
    const parsed = JSON.parse(text) as Record<string, unknown>
    if (typeof parsed !== 'object' || parsed === null) return null
    if (parsed.version !== PROFILE_VERSION) return null
    if (!Array.isArray(parsed.coreIds)) return null
    return parsed as unknown as ArchLensAnalysisProfile
  } catch {
    return null
  }
}
...
export function parseProfileObject(text: string): Record<string, unknown> | null {
 ...
 */
export function sanitizeCoreIds(index: CodeIndexResult, raw: unknown): string[] {
 ...
 */
export function buildProfileConceptTree(raw: unknown, idPrefix: string): ConceptTreeNode[] {
  return buildTree(raw, idPrefix, 0)
}

function buildTree(raw: unknown, idPrefix: string, depth: number): ConceptTreeNode[] {
  if (!Array.isArray(raw)) return []
  const out: ConceptTreeNode[] = []
  for (let i = 0; i < raw.length; i += 1) {
    const item = raw[i]
    if (typeof item !== 'object' || item === null) continue
    const record = item as { name?: unknown; desc?: unknown; inside?: unknown; children?: unknown }
...
function extractMermaidLocal(out: string): string {
 ...
 */
export function sanitizeFlow(raw: unknown, angle: FlowAngle): AnalysisFlow | undefined {
  if (typeof raw !== 'object' || raw === null) return undefined
  const record = raw as { title?: unknown; mermaid?: unknown }
  if (typeof record.mermaid !== 'string') return undefined
 ...
 */
export function sanitizeSeqMessages(raw: unknown, coreIds: readonly string[]): ArchLensSequenceMessage[] {
  if (!Array.isArray(raw)) return []
  const idSet = new Set(coreIds)
  const out: ArchLensSequenceMessage[] = []
  for (const item of raw) {
    if (typeof item !== 'object' || item === null) continue
 ...
 */
export function sanitizeEvents(raw: unknown): AnalysisEvent[] {
  if (!Array.isArray(raw)) return []
  const out: AnalysisEvent[] = []
  for (const item of raw) {
    if (typeof item !== 'object' || item === null) continue
    const record = item as { event?: unknown; mode?: unknown; producers?: unknown; consumers?: unknown; note?: unknown }
    if (typeof record.event !== 'string' || record.event.trim() === '') continue
    const producers = Array.isArray(record.producers)
      ? record.producers.filter((p): p is string => typeof p === 'string').slice(0, 8)
      : []
    const consumers = Array.isArray(record.consumers)
      ? record.consumers.filter((c): c is string => typeof c === 'string').slice(0, 8)
      : []
...
async function generateStructure(
  ...
  index: CodeIndexResult,
  language: string,
  want: { core: boolean; concept: boolean },
  signal?: AbortSignal,
): Promise<{ coreIds?: string[]; conceptTree?: ConceptTreeNode[] }> {
  try {
    const items: string[] = []
...
async function generateFigures(
  ...
  coreIds: string[],
  language: string,
  want: { flow: boolean; seq: boolean; events: boolean },
  signal?: AbortSignal,
): Promise<{ flow?: Partial<Record<FlowAngle, AnalysisFlow>>; seqMessages?: ArchLensSequenceMessage[]; events?: AnalysisEvent[] }> {
  try {
    const items: string[] = []
    if (want.flow) {
    ...
    if (want.flow) {
      ...
      const flows: Partial<Record<FlowAngle, AnalysisFlow>> = {}
      if (typeof rawFlow === 'object' && rawFlow !== null) {
        const record = rawFlow as Record<string, unknown>
        for (const angle of FLOW_ANGLES) {
          const flow = sanitizeFlow(record[angle], angle)
          if (flow !== undefined) flows[angle] = flow
        }
      }
...
export async function regenerateProfileField(
  ...
  index: CodeIndexResult,
  language: string,
  kind: 'concept' | 'core' | 'flow' | 'seq' | 'events',
  sandboxPolicy?: SandboxExecutionPolicy,
): Promise<ArchLensAnalysisProfile> {
  const key = `${root}\u0000${language}`
  ...
  const next = previous.catch(() => {}).then(async () => {
    ...
    const updated: ArchLensAnalysisProfile = { ...base }
    if (kind === 'core' || kind === 'concept') {
      ...
      if (kind === 'core') {
        ...
        if (structure.conceptTree === undefined) throw new Error('concept regeneration produced no tree')
      ...
      if (kind === 'flow') {
        ...
        if (figures.flow === undefined) throw new Error('flow regeneration produced no diagram')
        updated.flow = figures.flow
      } else if (kind === 'seq') {
        if (figures.seqMessages === undefined) throw new Error('seq regeneration produced no messages')
        updated.seqMessages = figures.seqMessages
      } else {
        if (figures.events === undefined) throw new Error('events regeneration produced no events')
