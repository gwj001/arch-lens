...
export async function analyzePackage(
...
): Promise<ArchLensCodeInsight> {
  const entryName = node.files.includes('index.ts')
    ? 'index.ts'
    : node.files.includes('index.js')
      ? 'index.js'
      : undefined
  if (entryName === undefined) return { id: node.id, provides: [], listens: [], tools: [], remotes: [] }
  let head = ''
  try {
  ...
  for (const match of head.matchAll(PROVIDE_PATTERN)) {
    ...
    if (key !== undefined) provides.add(key)
  }
  for (const match of head.matchAll(LISTEN_PATTERN)) {
    const event = match[1] ?? match[2]
    if (match[2] !== undefined) remotes.add(match[2])
    if (event !== undefined) listens.add(event)
  }
...
export async function analyzeWorkspace(
  ...
  const insights: ArchLensCodeInsight[] = []
