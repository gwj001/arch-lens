...
import { sectionText } from './sequence.ts'
...
export function isClaimDoc(rel: string): boolean {
...
export async function docClaimOutline(fs: FileSystem, root: string, docPath: string): Promise<string> {
  try {
    const info = await fs.stat(await fs.resolve(docPath))
    if (info === undefined || info.type !== 'file') return ''
    const text = (await fs.readText(await fs.resolve(docPath))).slice(0, DOC_READ_BYTES)
...
export async function collectClaimOutlines(fs: FileSystem, root: string, language: string): Promise<string> {
  try {
...
export async function extractConceptSection(fs: FileSystem, docPath: string, root: string): Promise<ConceptTreeNode[]> {
  const info = await fs.stat(await fs.resolve(docPath))
  if (info === undefined || info.type !== 'file') return []
  const text = (await fs.readText(await fs.resolve(docPath))).slice(0, DOC_READ_BYTES)
...
function extractTreeFromText(text: string, docPath: string, root: string): ConceptTreeNode[] {
