...
export const FLOW_ANGLE_LABEL: Record<FlowAngle, string> = {
  event: '事件驱动',
  pipeline: '数据管道',
}

/** Angle-specific instruction: what story the diagram must tell. */
export function flowAngleRule(angle: FlowAngle): string {
  switch (angle) {
    case 'event':
 ...
 */
export const FLOW_STYLE_RULES: string = [
 ...
 */
export function flowAngleRules(angle: FlowAngle): string {
