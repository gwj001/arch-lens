...
function cacheName(language: string, angle: FlowAngle): string {
...
function extractMermaid(out: string): string {
 ...
 */
async function transcodeFlow(ctx: Context, pseudo: string, language: string, signal?: AbortSignal): Promise<string> {
...
export async function generateFlowFromCode(
  ...
  try {
    ...
    const parsed = JSON.parse(out.slice(start, end + 1)) as { title?: unknown; mermaid?: unknown }
 ...
 */
export async function flowDiagram(
  ctx: Context,
  fs: FileSystem,
  root: string,
  index: CodeIndexResult,
