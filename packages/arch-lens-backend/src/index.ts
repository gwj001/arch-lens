...
export class ArchLensService extends TypertRemoteService {
  ...
  private pendingCustomFigure: { figId: string; figureId: string; text: string; language: string; stagedAt: number } | null = null
  ...
  async remoteCustomFigurePrompt(request: { text: string; figureId?: string; language?: string; context?: { blurbs?: Record<string, string> } }): Promise<{ figId: string; figureId: string; prompt: string } | { error: string }> {
    ...
    try {
      ...
      this.pendingCustomFigure = { figId, figureId, text, language, stagedAt: Date.now() }
      // One-shot staging: clear after 30 minutes even if the agent never
  ...
  async remoteFigureFollowUp(request: { kind: FollowUpKind; language?: string; angle?: FlowAngle; methodLevel?: boolean; followUp: string }): Promise<FollowUpResult | { error: string }> {
    ...
    try {
      const index = await this.indexWorkspaceShared(root)
      return await figureFollowUp(this.ctx, this.ctx.fs, root, index, { ...request, language: request.language ?? '中文' }, this.sessionPolicy())
    } catch (error) {
