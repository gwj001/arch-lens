...
import { emptyLlmStatsSnapshot, llmStatsAdopted, llmStatsSnapshot, hydrateLlmStats, recordLlmCall } from './llm-stats.ts'
...
/** One 🎨 动态出图 scene figure held in the per-root memory store. */
interface CustomFigureEntry {
  figureId: string
  title: string
  diagram: string
  summary: string
...
export class ArchLensService extends TypertRemoteService {
   ...
   * stagings. */
  private pendingNotes = new Map<string, PendingNote>()
   ...
   * mid-turn. */
  private pendingCustomFigures = new Map<string, { figId: string; figureId: string; text: string; language: string; stagedAt: number; root: string; usageStart?: SessionUsageSnapshot }>()
  ...
  private customFigures = new Map<string, Map<string, CustomFigureEntry>>()
  ...
  private ensureFigureMap(root: string): Map<string, CustomFigureEntry> {
  ...
  private recordSessionUsage(kind: string, label: string, stagedAt: number, usageStart: SessionUsageSnapshot | undefined, sessionId: string | null): void {
    const session = sessionId === null ? undefined : this.ctx.get('sessions')?.get(sessionId as SessionId)
  ...
  > {
    ...
    // New facts are in hand — now the destructive steps may run. Invalidate
    // ONLY this root's in-memory graph: other workspaces' caches stay valid
    // (their facts are untouched) — a pre-fix clear() dropped every root's
    // cache on one workspace's rescan (benign re-reads, but cross-workspace
    // interference all the same).
  ...
  private async adoptLlmStats(): Promise<void> {
    ...
    try {
      ...
      hydrateLlmStats(root, JSON.parse(text) as LlmStatsSnapshot)
    } catch {
  ...
  async remoteCustomFigurePrompt(request: { text: string; figureId?: string; language?: string }): Promise<{ figId: string; figureId: string; prompt: string } | { error: string }> {
    ...
    try {
      ...
      const existing = figureId !== undefined
        ? this.customFigures.get(root)?.get(figureId) ?? await this.readDrawFromDisk(root, figureId)
        : null
      ...
      this.pendingCustomFigures.set(figId, {
        figId, figureId, text, language, stagedAt: Date.now(), root,
        ...(usageStart !== undefined ? { usageStart } : {}),
      })
      // One-shot staging: clear after 30 minutes even if the agent never
      // answers (same defensive TTL as the figure/dynamic branches).
      setTimeout(() => { this.pendingCustomFigures.delete(figId) }, 30 * 60 * 1000)
  ...
  async remoteCustomFigure(request: { figureId?: string }): Promise<{ figureId: string; title: string; diagram: string; summary: string; text: string; saved?: boolean } | null | { error: string }> {
    ...
    for (const entry of this.customFigures.get(root)?.values() ?? []) {
  ...
  async remoteFigureRepairPrompt(request: { figureId: string; error: string }): Promise<{ figId: string; figureId: string; prompt: string } | { error: string }> {
    ...
    try {
      ...
      // the fresh figId and overwrites the scene entry (same scene, same
      // root), so a repair round needs no separate capture branch. text
      // preserved so the scene keeps its original request wording.
      this.pendingCustomFigures.set(figId, {
        figId, figureId: request.figureId, text: scene.text, language: '中文', stagedAt: Date.now(), root,
        ...(usageStart !== undefined ? { usageStart } : {}),
      })
      setTimeout(() => { this.pendingCustomFigures.delete(figId) }, 30 * 60 * 1000)
  ...
  async remoteNotePending(request: {
    ...
    // One-shot staging with a defensive TTL: if the answer never arrives the
    // entry cannot leak (same 30-min bound as the figure stagings).
    setTimeout(() => { this.pendingNotes.delete(stagedKey) }, 30 * 60 * 1000)
    return { ok: true }
  }
  ...
  protected async [Service.init](): Promise<void> {
    ...
    this.ctx.on('session/event', (session, event) => {
      ...
      // Custom figure (🎨 动态出图): an answer carrying a staged custom figId
      // is captured into the STAGING root's scene map (diagram + 概要) —
      ...
      // flips `saved` back to false (the disk copy is now stale). Attribution
      // is the staged root (the workspace whose facts built the prompt), NOT
      // the answering session's cwd — switching workspaces mid-turn must not
      // strand the scene in the wrong workspace.
      if (this.pendingCustomFigures.size > 0) {
        for (const stagedCustom of this.pendingCustomFigures.values()) {
      ...
      // must not pollute the learning notes. The staged entry is matched PER
      // SESSION (falling back to the anonymous no-session bucket) and only
      // the consuming entry is removed: another session's staged explain
      // stays intact for its own answer.
