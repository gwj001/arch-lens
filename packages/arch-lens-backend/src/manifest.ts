...
import type { FileSystem, FsTarget, FsDirEntry, FsInfo } from '@deepseek-ai/dsh-fs'
...
/** One discovered file: its cache-relative path and the resolved target. */
...
async function walk(fs: FileSystem, dirTarget: FsTarget, rel: string, out: WalkedFile[]): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    if (entry.type === 'directory') {
      if (SKIP_DIRS.has(entry.name) || SKIP_TEST_DIRS.has(entry.name) || childRel === CACHE_DIR || childRel.startsWith(`${CACHE_DIR}/`)) continue
...
async function readManifest(fs: FileSystem, root: string): Promise<WorkspaceManifest | null> {
  try {
    const target = await fs.resolve(MANIFEST_FILE, { cwd: root })
    const info = await fs.stat(target)
    if (info === undefined || info.type !== 'file') return null
    const parsed = JSON.parse(await fs.readText(target)) as WorkspaceManifest
...
async function writeManifest(fs: FileSystem, root: string, files: Record<string, ManifestEntry>, sandboxPolicy?: SandboxExecutionPolicy): Promise<void> {
  try {
    const manifest: WorkspaceManifest = { root, at: Date.now(), files }
    const target = await fs.resolve(MANIFEST_FILE, { cwd: root })
...
export async function compareWorkspaceChanges(fs: FileSystem, root: string): Promise<WorkspaceCompare> {
  ...
  for (const file of walked) {
    ...
    if (prev !== undefined && md5 !== undefined && prev.md5 === md5) {
      ...
      next[file.rel] = { ...prev, version: info.version as string }
      continue
    }
    changed = true
    if (prev === undefined) added.push(file.rel)
