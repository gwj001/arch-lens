 ...
 */
export const MERMAID_SYNTAX_RULE =
  '若使用 flowchart：subgraph 标题一律写成引号形式 `subgraph id["标题"]`——裸标题里的半角括号 ( ) 会导致整图渲染失败；'
 ...
 */
function repairEdgeLabels(source: string): string {
 ...
 */
export function quoteBareSubgraphTitles(source: string): string {
  return source.replace(BARE_SUBGRAPH_LINE, (line, head: string, id: string, open: string, title: string, close: string) => {
