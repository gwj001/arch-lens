...
export function trimToLimit(text: string): string {
  ...
  if (heads.length <= MAX_NOTE_ENTRIES) return text
 ...
 */
export function parseNotes(text: string): ParsedEntry[] {
...
export async function readNotes(
  ...
  root: string,
  notesFile: string,
): Promise<ArchLensNotesResult | { error: string }> {
  try {
    const target = await fs.resolve(notesFile, { cwd: root })
    const info = await fs.stat(target)
    if (info === undefined || info.type !== 'file') return { path: notesFile, entries: [] }
    const text = await fs.readText(target)
    const entries: ArchLensNoteEntry[] = parseNotes(text)
      .map(entry => {
