...
function callGraphToMermaid(edges, language) {
    ...
    const label = (count) => `${verb}${count > 1 ? `×${count}` : ''}`;
    const lines = ['%%{init: {"flowchart": {"curve": "linear", "nodeSpacing": 110, "rankSpacing": 170}}}%%', 'flowchart LR'];
    ...
    for (const edge of edges) {
        ...
        if (ab > 0 && ba > 0)
            lines.push(`  ${first} <-->|${label(ab + ba)}| ${second}`);
        else if (ab > 0)
            lines.push(`  ${first} -->|${label(ab)}| ${second}`);
        else
            lines.push(`  ${second} -->|${label(ba)}| ${first}`);
    }
...
export function ArchView(props) {
    ...
    const setAllMethods = (on) => {
        ...
        const view = on ? 'method' : 'entity';
        for (const id of METHOD_TABS)
            setMethodPersisted(id, on);
    ...
    const invalidateDynamicFigure = (kind, key) => {
        ...
        void directRemote('dynamicFigureFailed', { request: { kind, targetKey: key, language } }).then(result => {
            if (!('error' in result) && result.removed)
                setNotice(ui(language, 'dynamicCacheInvalidated'));
        }).catch(() => { });
    };
    ...
    const [drawRenderVerdict, setDrawRenderVerdict] = useState(null);
     ...
     */
    const [drawSelection, setDrawSelection] = useState({ figureId: '', items: [] });
    /** The chip list ONLY counts while it belongs to the scene on screen. */
    const currentSelectionItems = () => drawFig.figureId !== undefined && drawSelection.figureId === drawFig.figureId ? drawSelection.items : [];
    const addDrawSelection = (kind, label) => {
        const figureId = drawFig.figureId ?? '';
        const target = { kind, label };
        setDrawSelection(previous => previous.figureId !== figureId ? { figureId, items: [target] } : { figureId, items: withSelection(previous.items, target) });
     ...
     */
    const repairDrawFigure = () => {
        const figureId = drawFig.figureId;
        const verdict = drawRenderVerdict;
        if (figureId === undefined || drawFig.status !== 'ready' || drawFig.diagram === undefined)
            return;
        if (verdict === null || verdict.diagram !== drawFig.diagram || verdict.error === null)
            return;
        ...
        void directRemote('figureRepairPrompt', {
    ...
    const genDocs = () => {
        ...
        void unwrapRemote(archLens.generateDocs({ language })).then(result => {
            ...
            if (degradedN > 0)
                notice += uiT(language, 'genDocDegradedSuffix', { n: degradedN });
            if (skippedFigure.length > 0)
                notice += uiT(language, 'genDocFigureMissingSuffix', { n: skippedFigure.length });
            if (failed.length > 0)
                notice += uiT(language, 'genDocFailedSuffix', { list: failed.map(outcome => outcome.title).join('、') });
    ...
    if (error !== null) {
        ...
        const unitBodies = {
            ...
            currentSelectionItems().length > 0
                ? h('div', { className: css.drawChips }, h('span', { className: css.badge }, uiT(language, 'drawChipsScope', { id: drawFig.figureId ?? '' })), drawSelection.items.map(item => h('span', { key: `${item.kind}\u0000${item.label}`, className: css.drawChip }, h('span', { className: css.drawChipText }, `${selectionGlyph(item.kind)} ${item.label}`), h('button', {
                        ...
                        : h('div', null, drawFig.title !== undefined && drawFig.title !== ''
                            ...
                            ? h(MermaidView, {
                                ...
                                onRendered: () => {
                                    ...
                                    if (drawFig.figureId !== undefined)
                                        repairAttemptsRef.current.delete(drawFig.figureId);
                                },
        ...
        dynamicFig !== null && dynamicFig.kind !== 'overview'
            ...
            dynamicFig.status === 'ready' && dynamicFig.diagram !== undefined
                ...
                ? h('div', { className: css.dynBody }, h(MermaidView, { key: `dyn-${dynamicFig.key}`, source: dynamicFig.diagram, onRenderError: () => invalidateDynamicFigure(dynamicFig.kind, dynamicFig.key) }))
    ...
    notice !== null ? h('div', { className: css.notice }, notice) : null, llmStatsOpen && llmStats !== null
        ...
        })(), h('div', { style: { display: 'flex', gap: 6, marginBottom: 4 } }, h('button', { className: `${css.btn} ${allMethodsOn ? css.btnPrimary : ''}`, onClick: () => setAllMethods(true) }, ui(language, 'methodAllOn')), h('button', { className: `${css.btn} ${allMethodsOff ? css.btnPrimary : ''}`, onClick: () => setAllMethods(false) }, ui(language, 'methodAllOff')), h('span', { style: { fontSize: 10, color: '#888', alignSelf: 'center' } }, ui(language, 'methodHint'))), llmStats.records.map((record, index) => {
