...
export function isModernLayout(graph: { lang?: string | undefined } | null | undefined): boolean {
...
export function relPathOf(graph: { root: string }, node: { path: string }): string {
  const root = graph.root.replace(/\\/g, '/').replace(/\/+$/, '')
  const path = node.path.replace(/\\/g, '/').replace(/\/+$/, '')
  if (path === root || root === '') return root.split('/').filter(Boolean).at(-1) ?? path
  if (path.startsWith(`${root}/`)) return path.slice(root.length + 1)
  return path
}

/** Singular unit name for count copy, per scan language. */
export function scanUnitLabel(lang: string | undefined, pluralCount: number): string {
  if (lang === 'java') return `${pluralCount} 个模块`
  if (lang === 'python') return `${pluralCount} 个包`
