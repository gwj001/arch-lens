 ...
 */
export function hasSavedOverrides(config: Pick<ArchLensPromptConfig, 'overviewPrompt' | 'explainStyle'>): boolean {
  return (config.overviewPrompt !== undefined && config.overviewPrompt.trim() !== '')
    || (config.explainStyle !== undefined && config.explainStyle.trim() !== '')
}
 ...
 */
export function evidenceClause(entries?: readonly EvidenceEntry[], basis = '事实依据'): string {
 ...
 * @returns the question text.
 */
export function dataQuestion(title: string, data: unknown, explainStyle: string, language: string, evidence?: readonly EvidenceEntry[], basis?: string): string {
  let body = ''
  try {
