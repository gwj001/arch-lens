...
function fitLabel(text: string, maxPx: number, size: number): string {
  let w = 0
    ...
    h('svg', { className: css.svg, style: { minWidth: width, minHeight: height }, viewBox: `0 0 ${width} ${height}` },
      ...
      nodes.map(node => {
        ...
        return h('g', {
          ...
          h('title', null, node.desc === '' ? node.name : `${node.name}｜${node.desc}`),
          h('rect', {
            width: 220,
            height: 34,
            rx: 7,
          ...
          h('text', { x: 8, y: 15, fontSize: 12, fontWeight: 600, fill: '#333' }, `${prefix}${nameFitted}`),
          h('text', { x: 8, y: 29, fontSize: 10, fill: '#666' }, descFitted),
...
export function InteractionGraph(props: InteractionGraphProps): React.JSX.Element {
  ...
  const truncate = (text: string, maxPx: number): string => {
...
export function SequenceGraph(props: SequenceGraphProps): React.JSX.Element {
  ...
  const wrapEdgeLabel = (label: string): string[] => {
    const per = 22
  ...
  sequence.forEach((message, index) => {
    ...
    if (message.from === message.to) {
      ...
      elements.push(
        ...
        h('text', { key: `t${index}`, x: x1 + 40, y: y + 10, fontSize: 11, fill: '#445', onMouseEnter: onEnter, onMouseLeave: onLeave, onContextMenu: ask(`时序消息 ${message.from} → ${message.to}（${message.label}）`, 'edge') },
          lines.map((line, li) => h('tspan', { key: `l${li}`, x: x1 + 40, dy: li === 0 ? 0 : 12 }, line))),
      )
    } else {
      ...
      elements.push(
        ...
        h('text', { key: `t${index}`, x: direction > 0 ? x1 + 6 : x1 - 6, y: y - (lines.length > 1 ? 17 : 5), fontSize: 11, fill: '#445', textAnchor: direction > 0 ? 'start' : 'end', onMouseEnter: onEnter, onMouseLeave: onLeave, onContextMenu: msgAsk },
          lines.map((line, li) => h('tspan', { key: `l${li}`, x: direction > 0 ? x1 + 6 : x1 - 6, dy: li === 0 ? 0 : 12 }, line))),
      )
    }
    ...
    if (hovered === index && onDynamicRequest !== undefined) {
      elements.push(
        h('text', {
          ...
          onClick: (): void => onDynamicRequest({ from: message.from, to: message.to, label: message.label }),
