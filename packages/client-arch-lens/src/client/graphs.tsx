  ...
  sequence.forEach((message, index) => {
    ...
    if (message.from === message.to) {
      elements.push(
        ...
        h('text', { key: `t${index}`, x: x1 + 40, y: y + 10, fontSize: 11, fill: '#445', onMouseEnter: onEnter, onMouseLeave: onLeave, onContextMenu: ask(`时序消息 ${message.from} → ${message.to}（${message.label}）`, 'edge') }, message.label.slice(0, 34)),
      )
    } else {
      ...
      elements.push(
        ...
        h('text', { key: `t${index}`, x: direction > 0 ? x1 + 6 : x1 - 6, y: y - 5, fontSize: 11, fill: '#445', textAnchor: direction > 0 ? 'start' : 'end', onMouseEnter: onEnter, onMouseLeave: onLeave, onContextMenu: msgAsk },
