...
export function InsightsPanel(props: InsightsPanelProps): React.JSX.Element {
  const { insight } = props
  if (insight === undefined) {
    return h('div', { className: css.panel },
      h('div', { className: css.title }, '🔍 代码解析（从源码提取）'),
      h('div', { className: css.hint }, '无解析结果'),
    )
  }
  const rows: React.ReactNode[] = []
  const add = (label: string, values: string[]): void => {
    if (values.length === 0) return
    rows.push(h('div', { key: label, className: css.row },
      h('span', { className: css.kind }, label),
      h('span', { className: css.values }, values.join(', ')),
    ))
  }
  ...
  if (rows.length === 0) {
    return h('div', { className: css.panel },
      h('div', { className: css.title }, '🔍 代码解析（从源码提取）'),
      h('div', { className: css.hint }, '入口源码中未发现服务/事件/工具注册'),
    )
  }
  return h('div', { className: css.panel },
    h('div', { className: css.title }, '🔍 代码解析（从源码提取）'),
