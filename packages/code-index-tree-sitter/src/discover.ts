...
const EXTENSIONS: Record<Exclude<CodeLanguage, 'unknown'>, string[]> = {
  typescript: ['.ts', '.tsx'],
  python: ['.py'],
  java: ['.java'],
}
...
const MANIFESTS: Record<Exclude<CodeLanguage, 'unknown'>, string[]> = {
...
async function resolveUnder(fs: FileSystem, base: string, name: string): Promise<FsTarget | null> {
  try {
...
async function fileExists(fs: FileSystem, base: string, name: string): Promise<boolean> {
...
async function dirExists(fs: FileSystem, base: string, name: string): Promise<boolean> {
  const target = await resolveUnder(fs, base, name)
  if (target === null) return false
  try {
    const info = await fs.stat(target)
...
export async function detectLanguage(fs: FileSystem, root: string): Promise<CodeLanguage> {
  for (const candidate of MANIFESTS.typescript) {
    if (await fileExists(fs, root, candidate)) return 'typescript'
  }
  for (const candidate of MANIFESTS.python) {
    if (await fileExists(fs, root, candidate)) return 'python'
  }
  for (const candidate of MANIFESTS.java) {
    if (await fileExists(fs, root, candidate)) return 'java'
  }
  return 'unknown'
}
 ...
 */
export async function discoverPackageRoots(fs: FileSystem, root: string, language: CodeLanguage): Promise<string[]> {
...
async function listDirs(fs: FileSystem, base: string, rel: string): Promise<string[]> {
  try {
 ...
 */
export async function collectSources(fs: FileSystem, pkgDir: string, language: Exclude<CodeLanguage, 'unknown'>): Promise<FsTarget[]> {
  const extensions = EXTENSIONS[language]
  const files: FsTarget[] = []
  const walk = async (dir: FsTarget): Promise<void> => {
...
export async function readSmall(fs: FileSystem, target: FsTarget, maxBytes = 262144): Promise<string | null> {
  try {
    const info = await fs.stat(target)
    if (info === undefined || info.type !== 'file') return null
    if (info.size !== undefined && info.size > maxBytes) return null
 ...
 */
export async function manifestDeps(fs: FileSystem, pkgDir: string, language: Exclude<CodeLanguage, 'unknown'>): Promise<string[]> {
  const deps: string[] = []
  if (language === 'typescript') {
    ...
    if (meta !== null) {
      try {
        const parsed = JSON.parse(meta) as { dependencies?: Record<string, string>; peerDependencies?: Record<string, string> }
        for (const section of [parsed.dependencies, parsed.peerDependencies]) {
          if (section !== undefined) for (const name of Object.keys(section)) deps.push(name)
        }
      } catch {
