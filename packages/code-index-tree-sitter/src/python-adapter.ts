  ...
  for (const statement of statements) {
    if (statement.type === 'import_statement') {
      for (const dotted of collect(statement, 'dotted_name')) {
        if (dotted.text !== '') imports.push({ from: relPath, to: dotted.text, names: [] })
      }
      ...
      for (const child of statement.children) {
        ...
        if (child.type === 'aliased_import') {
          ...
          if (first !== undefined) names.push(first.text.split('.')[0] ?? first.text)
...
function entitiesOf(relPath: string, classes: TsNode[], functions: TsNode[]): CodeEntity[] {
  const entities: CodeEntity[] = []
  const classSpans: Array<{ start: number; end: number }> = []
 ...
 */
export function extractPython(relPath: string, source: string): { imports: CodeImport[]; entities: CodeEntity[] } {
